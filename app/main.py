...
# --- テーマ定義 ---
THEME_INFO = {
    "①": {"label":"売上・利益の予測精度を高める","problem":"着地見通しが担当者の勘に頼り、精度にばらつきがある","solution":"誰でも安定した予測が得られる未来予報システムを構築する","approach":"過去の月次データを用い、売上・利益の着地を自動計算して表示する"},
    "②": {"label":"月中でも業績の着地を把握する","problem":"月末まで利益着地が見えず、対策が後手になる","solution":"月中の累積データから見込みを先読みできる可視化を実現する","approach":"日々の速報値をもとに今月の着地を自動予測し表示する"},
    "③": {"label":"見えにくい赤字先をあぶり出す","problem":"一見好調でも利益が薄い先を感覚でしか把握できない","solution":"隠れ赤字の商品・顧客を自動発見し早期に警告する","approach":"売上・粗利・費用の構成を分析し損益悪化要因を可視化する"},
    "⑥": {"label":"異常な費用増加をすぐに察知する","problem":"月次費用の急増要因がレポートから分かりにくい","solution":"通常と異なる費用動向を自動検出して事前にアラートを出す","approach":"今月の費用構成を過去傾向と比較し異常をリアルタイム通知する"},
    "⑧": {"label":"予算と実績のズレを説明する","problem":"予実差の原因が明確でなく説明に時間がかかる","solution":"ズレ要因を自動分解し即答できる説明アシスタントを実装する","approach":"予算と実績の差を項目別に分解・可視化して因果を示す"},
