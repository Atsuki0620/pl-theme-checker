...
THEME_INFO = {
    "①": {
        ...
        "label": "売上・利益予測の精度向上",
        # 誰でも理解できる文章で 1～2 行
        "problem": "担当者ごとの勘に頼った着地見込みで、仕入量や在庫が過剰・不足する。",
        "task": "過去 P/L と季節変動を使い、自動で翌月の売上・利益を高精度に先読みする。",
        "approach": "P/L の時系列を機械学習（例：勾配ブースティング）で学習し、誤差を可視化して改善。"
    },
    "②": {
        # 24 文字
        "label": "月中段階での損益着地予測",
        "problem": "月末まで今月の利益が見えず、対策を打つ頃には手遅れになる。",
        "task": "受注・出荷実績が入った時点で営業利益の着地点をリアルタイムに推計する。",
        "approach": "受注／購買データから予測仕訳を生成し、ダッシュボードで最新着地を提示。"
    },
    "③": {
        # 25 文字
        "label": "利益を食う商品・顧客の特定",
        "problem": "売上は好調でも粗利を下げる“隠れ赤字”商品や顧客が把握できていない。",
        "task": "製品・顧客別に粗利率を算出し、値上げや撤退判断の根拠を示す。",
        "approach": "機械学習で収益性をスコアリングし、低粗利の要因（価格・販管費）を SHAP で可視化。"
    },
    "⑥": {
        # 20 文字
        "label": "費用比率の急変動を監視",
        "problem": "物流費や外注費が突発的に膨らむ月があり、原因特定に時間が掛かる。",
        "task": "月次費用構成のベースラインを作り、異常な跳ね上がりを即時検知する。",
        "approach": "費用比率に Isolation Forest を適用し、外れ値をアラート＋詳細ドリルダウン。"
    },
    "⑧": {
        # 23 文字
        "label": "予算と実績の差異要因分解",
        "problem": "差異分析が金額羅列で終わり、『結局なぜ？』に答えられない。",
        "task": "売上・原価・販管費のズレを数値分解し、経営会議で即説明できる状態に。",
        "approach": "決定木＋SHAP で差異寄与度を算出し、Waterfall グラフで可視化。"
    },
    "⑨": {
        # 26 文字
        "label": "現場KPIと利益の数値連携",
        "problem": "稼働率改善が利益にどれだけ効くか定量根拠がなく、投資判断が難しい。",
        "task": "現場 KPI と粗利・販管費の因果関係をモデル化し、改善 ROI を提示する。",
        "approach": "現場データと P/L を突合し、Granger 因果＋XGBoost で寄与度を算出。"
    },
    "⑩": {
        # 24 文字
        "label": "中計目標の達成可能性検証",
        "problem": "中期計画の KGI が『根拠薄弱』と批判され、修正を迫られている。",
        "task": "過去実績と外部指数から目標達成確率を数値で示し、説得力を高める。",
        "approach": "四半期 P/L と市況指数を ANN で回帰し、シナリオ別の目標到達率を算出。"
    },
}
